...
contract GlobalMarketConfig is RoleValidation {
    ...
    constructor(IMarketStorage _marketStorage, ILiquidityVault _liquidityVault, ITradeStorage _tradeStorage) RoleValidation(roleStorage) {
    ...
    function setMarketFundingConfig(bytes32 _marketKey, uint256 _fundingInterval, uint256 _maxFundingVelocity, uint256 _skewScale, uint256 _maxFundingRate) external onlyModerator {
    ...
    function setMarketBorrowingConfig(bytes32 _marketKey, uint256 _borrowingFactor, uint256 _borrowingExponent, bool _feeForSmallerSide) external onlyModerator {
        address market = marketStorage.getMarket(_marketKey).market;
    ...
    function setMarketPriceImpactConfig(bytes32 _marketKey, uint256 _priceImpactFactor, uint256 _priceImpactExponent) external onlyModerator {
        address market = marketStorage.getMarket(_marketKey).market;
