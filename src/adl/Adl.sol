...
contract Adl is RoleValidation {
    ...
    constructor(address _tradeStorage, address _priceFeed, address _liquidityVault, address _roleStorage)
    ...
    function flagForAdl(IMarket market, bool _isLong) external onlyAdlKeeper {
        // require(market != Market(address(0)), "ADL: Invalid market");
        // // get current price
        ...
        // int256 pnlFactor = MarketUtils.getPnlFactor(market, price, baseUnit, _isLong);
        // // fetch max pnl to pool ratio
        // uint256 maxPnlFactor = market.maxPnlFactor();
    ...
    function executeAdl(IMarket market, uint256 _sizeDelta, bytes32 _positionKey, bool _isLong)
        external
        onlyAdlKeeper
    {
        ...
        // cache.indexPrice = priceOracle.getPrice(market.indexToken());
        // cache.indexBaseUnit = dataOracle.getBaseUnits(market.indexToken());
        // // Get token prices
        ...
        int256 startingPnlFactor =
            MarketUtils.getPnlFactor(market, liquidityVault, cache.indexPrice, cache.indexBaseUnit, _isLong);
